...
<tr>
<td colspan="3">
<strong>Fable wrap-up</strong>, task 59, 
<code>java-databricks-cluster-policy</code>. 
The skill won 12 rules, the baseline won 4, and 9 were ties.
</td>
</tr>
<tr>
<td valign="top" width="33%">Done — `java TestMain.java` passes all<br>89 checks. The only changes are two new<br>files, `PolicyReconciler.java` and<br>`ReconcileReport.java`; TestMain.java,<br>everything under docs/, and the existing<br>client code are untouched, and no<br>scratch files were left behind.<br><br>**Research findings (done before any<br>code change).** The live first-party<br>docs agree with the pinned fixtures in<br>`docs/contract.json` and<br>`docs/official_sources.json` — no<br>conflict to report. Specifically:<br><br>- The cluster-policy `list()` documents<br>only `sort_column` (POLICY_CREATION_TIME<br>| POLICY_NAME) and `sort_order` (ASC |<br>DESC) — no `max_results`/`page_token` —<br>and the Java SDK's<br>`ListPoliciesResponse` has a single<br>`policies` field with no<br>`next_page_token` or `total_count`. So<br>yes: the runbook assumption is stale in<br>the other direction people feared — the<br>policy list really is one unpaginated<br>`{policies}` document.<br>- The policy<br><br>[...]</td>
<td valign="top" width="33%">Implemented the reconciler and verified<br>it with the acceptance harness.<br><br>`java TestMain.java` passes all 89<br>checks.<br><br>Changes made:<br>- Added `PolicyReconciler.java`<br>- Added `ReconcileReport.java`<br><br>No existing files were changed:<br>- `PoliciesClient.java` is untouched<br>- `Json.java` is untouched<br>- `TestMain.java` is untouched<br>- Everything under `docs/` is untouched<br>- No scratch files were left behind<br><br>Research was completed before code<br>changes. The current official Databricks<br>sources agree with the pinned fixtures<br>in `docs/contract.json` and<br>`docs/official_sources.json`; I found no<br>conflict.<br><br>Key documentation findings:<br>- Cluster policy list is unpaginated. It<br>supports `sort_column`<br>(`POLICY_CREATION_TIME` or<br>`POLICY_NAME`) and `sort_order` (`ASC`<br>or `DESC`). It does not use<br>`max_results` or `page_token`. The Java<br>SDK `ListPoliciesResponse` contains only<br>`policies`, with no `next_page_token` o<br><br>[...]</td>
<td valign="top" width="33%">`java TestMain.java` passes all 89<br>checks. I added two files,<br>`PolicyReconciler.java` and<br>`ReconcileReport.java`. I did not change<br>`TestMain.java`, anything under `docs/`,<br>or the existing client code.<br><br>I checked the current official<br>Databricks sources before changing code.<br>The live sources agree with the pinned<br>fixtures in `docs/contract.json` and<br>`docs/official_sources.json`, so there<br>is no fixture conflict to report.<br><br>The Databricks cluster policy list<br>endpoint is not paginated. The cluster<br>policy `list()` documentation only<br>includes `sort_column` and `sort_order`.<br>It does not include `max_results` or<br>`page_token`. The Java SDK type<br>`ListPoliciesResponse` only has a<br>`policies` field, with no<br>`next_page_token` or `total_count`.<br><br>The Databricks policy families list<br>endpoint is paginated. It takes<br>`max_results` and `page_token`, and it<br>returns `next_page_token`. The Go SDK<br>loops on `NextP<br><br>[...]</td>
</tr>
</tbody>
</table>
